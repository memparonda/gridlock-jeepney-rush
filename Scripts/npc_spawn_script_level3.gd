...
@export var npc_pool := [
	...
	{"scene": preload("res://Scenes/Npc/npc_police_1.tscn"), "weight": 20}
]
...
func _ready():
	var path_length = curve.get_baked_length()
	var spacing = path_length / 80.0  # evenly distribute. MUST BE SAME VALUE AS i IN RANGE!
	
	for i in range(80):
		var offset = i * spacing # MUST BE SAME VALUE AS SPACING!
		spawn_npc(offset)
